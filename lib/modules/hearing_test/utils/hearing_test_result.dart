...
class HearingTestResult {
  ...
  Map<String, List<bool?>> getFrequenciesThatRequireMasking() {
    List<bool?> leftEarResultsThatNeedMasking = List<bool?>.filled(
      HearingTestConstants.TEST_FREQUENCIES.length,
      null,
    );
    List<bool?> rightEarResultsThatNeedMasking = List<bool?>.filled(
      HearingTestConstants.TEST_FREQUENCIES.length,
      null,
    );
    for (int i = 0; i < HearingTestConstants.TEST_FREQUENCIES.length; i++) {
      if (leftEarResults[i] != null && rightEarResults[i] != null) {
        ...
        leftEarResultsThatNeedMasking[i] =
            leftValue - rightValue >= threshold;
        rightEarResultsThatNeedMasking[i] =
            rightValue - leftValue >= threshold;
      }
    }
    return {
      'left': leftEarResultsThatNeedMasking,
      'right': rightEarResultsThatNeedMasking,
    };
  }
}
